...
- [ ] **Firebase Room Migration & Admin Editor**: Migrate all static room definitions to Firestore. Implement a **Hybrid/Background Prefetching Engine** to fetch the active room and pre-cache its 6 adjacent neighbors (N, S, E, W, Up, Down). Provide a split-screen **Visual Admin Editor** showing the active 5x5 sub-grid alongside a **3D Adjacent Map Visualizer** with color-coded alerts (green/yellow/red) to validate exit-to-entry tile alignment. Store custom entities in **Normalized Collections** (`global_items`, `global_npcs`, `global_abilities`) with client-side joins. Secure Firestore writes strictly behind **Custom User Claims** (`admin: true`) and Firestore rules. Support local developer environments via **Firebase Local Emulator Suite** integrations and a NodeJS automated seeding script (`npm run seed:local`).